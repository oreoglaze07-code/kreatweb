  ...
  return (
    ...
    <script>
      ...
      function renderCalendar() {
        ...
        html += '<span class="cal-month">' + months[currentMonth] + ' ' + currentYear + '</span>';
        html += '<button class="cal-nav" onclick="nextMonth()">›</button>';
        html += '</div>';
        html += '<div class="cal-days"><span>Su</span><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span></div>';
        ...
        for (var d = 1; d <= lastDay; d++) {
          ...
          html += '<button class="cal-day' + (past ? ' disabled' : '') + (sel ? ' selected' : '') + '"' + (past ? ' disabled' : ' onclick="pickDate(' + currentYear + ',' + currentMonth + ',' + d + ')"') + '>' + d + '</button>';
        }
        html += '</div>';
        cal.innerHTML = html;
      }
      ...
      function pickDate(y, m, d) {
        ...
        var months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
        document.getElementById('dateDisplay').textContent = months[m] + ' ' + d + ', ' + y;
        document.getElementById('calendarPopup').style.display = 'none';
        selectedTime = null;
        showTimeSlots();
        updateSendBtn();
      ...
      function showTimeSlots() {
        ...
        times.forEach(function(t) {
          var sel = selectedTime === t;
          html += '<button class="time-btn' + (sel ? ' selected' : '') + '" onclick="pickTime(this, \\'' + t + '\\')">' + t + '</button>';
        });
        grid.innerHTML = html;
      }
      ...
      function updateSendBtn() {
        document.getElementById('sendBtn').disabled = !(selectedDate && selectedTime);
      }
